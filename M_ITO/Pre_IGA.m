...
Ele.CtrPtsNumU = NURBS.order(1); Ele.CtrPtsNumV = NURBS.order(2);
    ...
    % Debug info: print Ele.CtrPtsCon information
    fprintf('=== Pre_IGA Debug Info ===\n');
    fprintf('Ele.Num = %d, Ele.CtrPtsNum = %d\n', Ele.Num, Ele.CtrPtsNum);
    fprintf('Ele.CtrPtsCon size: [%d, %d]\n', size(Ele.CtrPtsCon,1), size(Ele.CtrPtsCon,2));
    if Ele.Num > 0
        fprintf('Ele.CtrPtsCon(1,:) = %s\n', mat2str(Ele.CtrPtsCon(1,:)));
        if Ele.Num > 1
            fprintf('Ele.CtrPtsCon(2,:) = %s\n', mat2str(Ele.CtrPtsCon(2,:)));
        end
    end
    fprintf('==========================\n\n');
